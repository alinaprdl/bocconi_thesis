...
OUTPUT_DIR = "txt_files_cleaned_KF"
OUTPUT_EXCEL = "cleanup_KF_log.xlsx"
...
for row_idx, row in enumerate(ws_input.iter_rows(min_row=2, values_only=False), start=2):
    try:
        ...
        # Construct raw text file path (Doc_ID already includes firm prefix like "KF_01")
        ...
        # Find end point: "Our Expert(s)", "Related Articles", "Follow Us", "Featured Insights", "Want to Learn More?", or "Related Insights" (whichever comes first)
        # Case-insensitive, but must be alone on one line
        our_experts_pattern = r'^Our Experts?$'  # Matches both "Our Expert" and "Our Experts"
        related_articles_pattern = r'^Related Articles$'
        follow_us_pattern = r'^Follow Us$'
        featured_insights_pattern = r'^Featured Insights$'
        want_to_learn_pattern = r'^Want to Learn More\?$'
        related_insights_pattern = r'^Related Insights$'
        
        our_experts_match = re.search(our_experts_pattern, body_text_from_start, re.MULTILINE | re.IGNORECASE)
        related_articles_match = re.search(related_articles_pattern, body_text_from_start, re.MULTILINE | re.IGNORECASE)
        follow_us_match = re.search(follow_us_pattern, body_text_from_start, re.MULTILINE | re.IGNORECASE)
        featured_insights_match = re.search(featured_insights_pattern, body_text_from_start, re.MULTILINE | re.IGNORECASE)
        want_to_learn_match = re.search(want_to_learn_pattern, body_text_from_start, re.MULTILINE | re.IGNORECASE)
        ...
        matches = []
        if our_experts_match:
            matches.append(("Our Expert(s)", our_experts_match.start()))
        if related_articles_match:
            matches.append(("Related Articles", related_articles_match.start()))
        if follow_us_match:
            matches.append(("Follow Us", follow_us_match.start()))
        if featured_insights_match:
            matches.append(("Featured Insights", featured_insights_match.start()))
        if want_to_learn_match:
            matches.append(("Want to Learn More?", want_to_learn_match.start()))
        ...
        if matches:
            ...
            warnings.append("No end marker found (Our Expert(s)/Related Articles/Follow Us/Featured Insights/Want to Learn More?/Related Insights)")
...
print(f"Skipped (non-KF): {skipped}")
